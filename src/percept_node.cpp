...
public:
    ...
    WorldInfo(ros::NodeHandle& nh)
    {
        ...
        client_map_generator_ = nh.serviceClient<world_percept_assig3::UpdateObjectList>(srv_update_obj_name_);

        // create a client for the reasoning node service to assert knowledge
        srv_assert_knowledge_name_ = "assert_knowledge";
        client_reasoning_ = nh.serviceClient<world_percept_assig3::UpdateObjectList>(srv_assert_knowledge_name_);
    ...
    for (int i = 0; i < msg->name.size(); i++)
    {
        ...
        if (d<1.1)
        {
            ...
            if (it == v_seen_obj_.end()) {
                ...
                world_percept_assig3::UpdateObjectList srv_reasoning;
